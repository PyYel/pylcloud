...
def _config_logger(
    ...
    logs_output: list[str] = ["console", "file"] if os.getenv("LOGS_DIR", None) else ["console"]
    ):
    """
    ...
    if not logger.hasHandlers():
        ...
        if "file" in logs_output:
            file_handler = logging.FileHandler(os.path.join(logs_dir, f"{datetime.now().strftime('%H-%M-%S')}.log"))
